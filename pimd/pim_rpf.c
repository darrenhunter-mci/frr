		...
		const char *caller)
{
	...
	if (!pim_nht_lookup_ecmp(pim, &rpf->source_nexthop, src, &grp, neigh_needed)) {
		...
		if (!pim_addr_is_any(up->sg.src) && !I_am_RP(pim, up->sg.grp) &&
			...
			if (pim_nht_lookup_ecmp(pim, &nnh, src, &grp, false) &&
				...
				if (PIM_DEBUG_ZEBRA)
					zlog_debug("%s: multicast-only shortcut: resolving %pPAs on %s (RPF-failure path)",
						   __func__, &up->sg.src, nnh.interface->name);
	 ...
	 *
	 * Two-part gate:
	 *  - !I_am_RP: the RP (DMVPN hub) never wants a source SPT; its own
	 *    resolution would install a recursive /32 shadowing the BGP route and
	 *    break hub-relay RPF.
	 *  - switchover != PIM_SPT_INFINITY: a node pinned to the shared tree
	 *    (`spt-switchover infinity-and-beyond`, i.e. plain hub-relay with no
	 *    spoke<->spoke adjacency configured) must NOT trigger either -- the
	 *    resolved /32 would be recursive-and-unresolvable there and break the
	 *    receiver's OWN RPF. Only a spoke that has dropped the pin (Plan A's
	 *    A5, paired with A3's dynamic-map adjacency) both wants the SPT and can
	 *    resolve the /32, so only it triggers. The trigger then self-terminates
	 *    once the direct SPT switches over.
	 */
	if (!pim_addr_is_any(up->sg.src) && rpf->source_nexthop.interface) {
		...
		if (rpf_ifp && rpf_ifp->pim_nbma_enable &&
		    ...
		    pim->spt.switchover != PIM_SPT_INFINITY) {
			if (PIM_DEBUG_ZEBRA)
				zlog_debug("%s: multicast-only shortcut: resolving %pPAs on %s",
					   __func__, &up->sg.src, rpf->source_nexthop.interface->name);
